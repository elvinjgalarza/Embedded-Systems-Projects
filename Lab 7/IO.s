...
IO_Init
	...
	LDR R1, [R0]
	ORR R1, #0x01				;SET PUR FOR SW2 because negative logic
	STR R1, [R0]
...
IO_HeartBeat
	...
	LDR R0, [R1]
	EOR R0, R0, #0x02				;toggles bit for heatbeat to work
    STR R0, [R1]
...
loop	LDR R1, =GPIO_PORTF_DATA_R
		LDR R0, [R1]
		ANDS R0, R0, #SW2
		BNE loop						;if switch not pressed, keep checking
		MOV R0, #1						;if pressed, fall through and put LR onto stack to save PC before Delay1ms
		PUSH {LR}
		BL Delay1ms
		POP {LR}						
